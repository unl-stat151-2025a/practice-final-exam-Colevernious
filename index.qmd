...
```{r}
sum(yarn$machine_washable, na.rm = T)
```
    2. (3 points) Explain how your code works, including any changes in variable types that occur during the computation.

The original data type is boonlean (true/false). When I take the sum, R or python converts boonleans to an integer (1/0 respectively). NA values cannot be added in either language, so I first remove them (or replace NA values with false) and then take the sum of the converted integers. This results in the answer we got.
...
highest <- names(na)[which.max(na)]
fewest <- names(na)[which.min(na)]
median_columns <- names(na)[na == median]
highest
fewest
median_columns
```
...
ggplot(common_yarn_weight, aes(y = yarn_company_name, x = yarn_weight_name, fill = n)) + geom_tile()
...
5. Consider the columns `texture` and `texture_clean`. 
    ...
    > The code book and cleaning script say that 'stringr' 'str_trim' and 'str_to_lower'
    ...
    > Test to see if a descriptor in 'top_descriptors' is in texture_clean using 'str_detect'. If str_detect
...
6. Consider the variable `yarn_weight_wpi`.
    ...
    > 1. Replace all "0-4" values with 4 and all "5-6" values with 6, since range of values is disjoint
...
convert_weight_int(c("8", "0-4", "5-6", "7", "10", "11", "14", NA))
